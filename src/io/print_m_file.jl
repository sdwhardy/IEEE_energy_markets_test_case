...
function topology_df_IEEE(rt_ex, relax, _ac)
	...
	ppf_mainACDCStorage2mfile_IEEE(rem_df,ac_cable_df,dc_cable_df,rt_ex, relax, _ac)
end

#ACDC network with storage main logic to create topology.m file
function ppf_mainACDCStorage2mfile_IEEE(r_df,ac_df, dc_df, cdir,relax, _ac)
	base_mva=100
	matfile = open(cdir*"topology.m","w")#open the .mat file
	ppf_header(matfile,base_mva)#print top function data
	ppf_Buss_IEEE(matfile,r_df)#prints the bus data
	ppf_Gens_IEEE(matfile,r_df)#prints all generator (OWPP) data
	ppf_acbranches_IEEE(matfile,ac_df,relax, _ac)
	ppf_costs_IEEE(matfile,r_df)
	ppf_BussDC_IEEE(matfile,r_df)#prints the bus data
	ppf_BussDC_ne_blank(matfile)#prints the bus data
	#ppf_dcbranches_blank(matfile)
	ppf_dcbranches_IEEE(matfile,dc_df, relax)
	ppf_convs_IEEE(matfile,r_df)
	ppf_dc_blank_conv_ne_IEEE(matfile)
	ppf_storage_IEEE(matfile,r_df)
...
function ppf_Buss_IEEE(mf,r_df)
	println(mf,"%% bus data
...
function ppf_Gens_IEEE(mf,r_df)
...
function ppf_acbranches_IEEE(mf,ac_df, relax, _AC)
...
function ppf_costs_IEEE(mf,r_df)
	println(mf,"%%-----  OPF Data  -----%%
	%% generator cost data
...
function ppf_BussDC_IEEE(mf,r_df)
...
function ppf_dcbranches_IEEE(mf,dc_df, relax)
...
function ppf_convs_IEEE(mf,r_df)
	println(mf,"
	%% existing converters
...
function ppf_dc_blank_conv_ne_IEEE(mf)
	println(mf,"
...
function ppf_storage_IEEE(mf,r_df)
	println(mf, " % hours
